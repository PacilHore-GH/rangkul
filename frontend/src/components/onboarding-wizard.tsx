...
export function OnboardingWizard() {
  ...
  function next() {
    ...
    if (step === 1 && birthYear && (+birthYear < 1900 || +birthYear > 2026)) return setError("Masukkan tahun lahir antara 1900 dan 2026.");
    if (step === 2 && selected.length === 0) return setError("Pilih minimal satu kebutuhan dukungan untuk melanjutkan.");
    setStep(step + 1);
  }
  ...
  async function finish() {
    ...
    setLoading(true);
    try {
      await api("/people/onboarding", { method: "POST", body: JSON.stringify({
  ...
  return <main className="wizard-shell"><section className="wizard-card">
    ...
    {step === 1 && <><h1>Siapa yang akan didampingi?</h1><p className="muted">Gunakan nama panggilan agar terasa nyaman dan privat.</p><label>Nama panggilan<input className="field" value={displayName} onChange={e => setDisplayName(e.target.value)} maxLength={100} autoFocus /></label><label>Tahun lahir <span className="muted">(opsional)</span><input className="field" inputMode="numeric" value={birthYear} onChange={e => setBirthYear(e.target.value.replace(/\D/g, "").slice(0, 4))} placeholder="Contoh: 2020" /></label></>}
