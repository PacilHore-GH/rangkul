...
def strong_password(length: int) -> str:
    return "Aa1!" + "x" * (length - 4)


@pytest.mark.parametrize("password, expected", [(strong_password(11), 422), (strong_password(12), 201), (strong_password(13), 201), (strong_password(127), 201), (strong_password(128), 201), (strong_password(129), 422)])
...
@pytest.mark.parametrize("password", ["a" * 12, "A" * 12, "Aa" * 6, "Aa1" * 4, "Aa1!aaaa aaa"])
def test_register_rejects_weak_password_combinations(client, password):
    http, _ = client
    assert register(http, email=f"weak{abs(hash(password))}@example.com", password=password).status_code == 422
...
def test_onboarding_input_bva_and_unhappy_cases(client, payload, expected):
    ...
    assert http.post("/api/v1/people/onboarding", json=payload).status_code == expected
